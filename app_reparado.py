...
def safe_read_csv(path: Path, columns: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        # If the file is corrupted, return a clean frame instead of crashing the app
        return pd.DataFrame(columns=columns)
...
def get_auth_conn() -> sqlite3.Connection:
    ensure_storage_ready()
    return sqlite3.connect(AUTH_DB, timeout=10)
...
def load_products() -> pd.DataFrame:
    df = safe_read_csv(PRODUCTS_FILE, PRODUCT_COLUMNS)
    if df.empty:
...
def load_categories() -> pd.DataFrame:
    df = safe_read_csv(CATEGORIES_FILE, CATEGORY_COLUMNS)
    if df.empty:
...
def load_sales() -> pd.DataFrame:
    df = safe_read_csv(SALES_FILE, SALES_COLUMNS)
    if df.empty:
...
def load_monthly_costs() -> pd.DataFrame:
    df = safe_read_csv(COSTS_FILE, COSTS_COLUMNS)
    if df.empty:
...
def generate_excel_file(products_df: pd.DataFrame, sales_df: pd.DataFrame, costs_df: pd.DataFrame) -> bytes | None:
...
def dashboard_tab() -> None:
    ...
    if excel_bytes is None:
        st.warning("No fue posible generar el Excel. Verifica que openpyxl esté instalado.")
    else:
        st.download_button(
...
if __name__ == "__main__":
    try:
        ...
        st.set_page_config(page_title="Inventario y Flujo de Caja", layout="wide")
        st.error("La aplicación encontró un error al iniciar.")
        st.exception(exc)
