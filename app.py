...
def inventory_tab() -> None:
    ...
    with st.form("add_product_form", clear_on_submit=True):
        ...
        category = c2.text_input("Categoría")
        ...
        if submitted:
            if not name.strip():
                ...
                add_product(name.strip(), category.strip(), int(stock), float(cost_price), float(sale_price))
...
def sales_tab() -> None:
    ...
    with st.form("sales_form", clear_on_submit=True):
        ...
        quantity = s2.number_input("Cantidad vendida", min_value=1, step=1)
        sale_dt = s3.date_input("Fecha de venta", value=date.today())
