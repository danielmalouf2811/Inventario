...
# ---------- Streamlit Cloud compatible storage ----------
# En Streamlit Cloud el sistema de archivos es efímero.
# Usamos st.cache_resource para mantener el directorio durante la sesión del servidor.
 
@st.cache_resource
def get_data_dir() -> Path:
    """Retorna un directorio de datos compartido durante la sesión del servidor."""
    data_dir = Path(tempfile.gettempdir()) / "streamlit_inventario_data"
...
def get_paths():
    ...
    return {
        ...
        "costs": DATA_DIR / "monthly_costs.csv",
...
SALES_COLUMNS = [
    "id", "sale_date", "product_id", "product_name", "product_category",
    "quantity", "unit_cost", "unit_sale_price", "subtotal", "pays_tax",
    "iva", "total_with_tax", "cost", "profit_before_isr", "isr", "net_profit",
]
...
def get_auth_conn() -> sqlite3.Connection:
    return sqlite3.connect(get_paths()["auth_db"])
...
def ensure_data_files() -> None:
    ...
    if not paths["products"].exists():
        pd.DataFrame(columns=PRODUCT_COLUMNS).to_csv(paths["products"], index=False)
    if not paths["sales"].exists():
        pd.DataFrame(columns=SALES_COLUMNS).to_csv(paths["sales"], index=False)
    if not paths["costs"].exists():
        pd.DataFrame(columns=COSTS_COLUMNS).to_csv(paths["costs"], index=False)
    if not paths["categories"].exists():
        pd.DataFrame(columns=CATEGORY_COLUMNS).to_csv(paths["categories"], index=False)
 
 
def load_products() -> pd.DataFrame:
    df = pd.read_csv(get_paths()["products"])
    if df.empty:
...
def load_categories() -> pd.DataFrame:
    df = pd.read_csv(get_paths()["categories"])
    if df.empty:
...
def save_categories(df: pd.DataFrame) -> None:
    df[CATEGORY_COLUMNS].to_csv(get_paths()["categories"], index=False)
...
def save_products(df: pd.DataFrame) -> None:
    df[PRODUCT_COLUMNS].to_csv(get_paths()["products"], index=False)
 
 
def load_sales() -> pd.DataFrame:
    df = pd.read_csv(get_paths()["sales"])
    if df.empty:
    ...
    defaults = {
        "product_category": "", "subtotal": 0.0, "pays_tax": False,
        "iva": 0.0, "total_with_tax": 0.0, "profit_before_isr": 0.0,
        "isr": 0.0, "net_profit": 0.0,
    }
    for col, default in defaults.items():
        if col not in df.columns:
            df[col] = default
    numeric_cols = [
        "id", "product_id", "quantity", "unit_cost", "unit_sale_price",
        "subtotal", "iva", "total_with_tax", "cost", "profit_before_isr", "isr", "net_profit",
    ]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
    df["id"] = df["id"].astype(int)
...
def save_sales(df: pd.DataFrame) -> None:
    ...
    df_to_save[SALES_COLUMNS].to_csv(get_paths()["sales"], index=False)
 
 
def load_monthly_costs() -> pd.DataFrame:
    df = pd.read_csv(get_paths()["costs"])
    if df.empty:
...
def save_monthly_costs(df: pd.DataFrame) -> None:
    df[COSTS_COLUMNS].to_csv(get_paths()["costs"], index=False)
...
def add_product(name: str, category: str, stock: int, cost_price: float, sale_price: float) -> None:
    ...
    new_row = {
        ...
        "stock": int(stock), "cost_price": float(cost_price),
        "sale_price": float(sale_price), "updated_at": datetime.utcnow().isoformat(),
    }
...
def update_product(product_id: int, name: str, category: str, stock: int, cost_price: float, sale_price: float) -> None:
    ...
    products.loc[idx, ["name", "category", "stock", "cost_price", "sale_price", "updated_at"]] = [
        name, category, int(stock), float(cost_price), float(sale_price), datetime.utcnow().isoformat(),
    ]
    save_products(products)
...
def register_sale(product_id: int, quantity: int, sale_dt: date, pays_tax: bool) -> tuple[bool, str]:
    ...
    sale_row = {
        "id": next_id(sales), "sale_date": sale_dt.isoformat(),
        "product_id": int(product["id"]), "product_name": product["name"],
        "product_category": product["category"], "quantity": quantity,
        "unit_cost": unit_cost, "unit_sale_price": unit_price,
        "subtotal": subtotal, "pays_tax": bool(pays_tax), "iva": iva,
        "total_with_tax": total_with_tax, "cost": cost,
        "profit_before_isr": profit_before_isr, "isr": isr, "net_profit": net_profit,
    }
...
def upsert_monthly_cost(year_month: str, fixed_cost: float, variable_cost: float, notes: str) -> None:
    ...
    if existing.any():
        costs.loc[existing, ["fixed_cost", "variable_cost", "notes", "updated_at"]] = [
            float(fixed_cost), float(variable_cost), notes, now,
        ]
    else:
        row = {
            "year_month": year_month, "fixed_cost": float(fixed_cost),
            "variable_cost": float(variable_cost), "notes": notes, "updated_at": now,
        }
...
def build_daily_summary(sales_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = (
        ...
        .agg(
            ventas_brutas=("subtotal", "sum"), iva=("iva", "sum"),
            ingresos_netos_sin_iva=("subtotal", "sum"), costo_ventas=("cost", "sum"),
            utilidad_antes_isr=("profit_before_isr", "sum"), isr=("isr", "sum"),
...
def build_monthly_summary(sales_df: pd.DataFrame, costs_df: pd.DataFrame) -> pd.DataFrame:
    monthly_sales = pd.DataFrame(
        columns=["year_month", "ventas_brutas", "iva", "ingresos_netos_sin_iva", "costo_ventas", "utilidad_bruta", "ventas_con_iva", "ventas_sin_iva"]
    )
    ...
    if monthly_sales.empty and costs_df.empty:
        return pd.DataFrame(
            columns=["year_month", "ventas_brutas", "ventas_con_iva", "ventas_sin_iva", "iva", "ingresos_netos_sin_iva", "costo_ventas", "utilidad_bruta", "costos_fijos", "costos_variables", "utilidad_operativa", "utilidad_antes_isr", "isr", "utilidad_neta"]
        )
    ...
    return merged[["year_month", "ventas_brutas", "ventas_con_iva", "ventas_sin_iva", "iva", "ingresos_netos_sin_iva", "costo_ventas", "utilidad_bruta", "costos_fijos", "costos_variables", "utilidad_operativa", "utilidad_antes_isr", "isr", "utilidad_neta"]].sort_values("year_month")
 
 
def build_inventory_export(products_df: pd.DataFrame) -> pd.DataFrame:
    if products_df.empty:
        return pd.DataFrame(columns=["Fecha", "Producto", "Categoría", "Stock", "Costo_unitario", "Precio_venta", "Valor_inventario"])
    ...
    return out[["Fecha", "Producto", "Categoría", "Stock", "Costo_unitario", "Precio_venta", "Valor_inventario"]].sort_values(["Fecha", "Producto"])
...
def build_cashflow_export(sales_df: pd.DataFrame, costs_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not sales_df.empty:
        ...
        frames.append(sales[["Fecha", "Tipo", "Categoría", "Descripción", "Cantidad", "Precio_unitario", "Total", "Mes", "Año", "paga_impuestos", "iva", "subtotal", "total_con_iva", "utilidad_antes_isr", "isr", "utilidad_neta"]])
    if not costs_df.empty:
        costs = costs_df.copy()
        period_date = pd.to_datetime(costs["year_month"] + "-01", errors="coerce")
        fixed = pd.DataFrame({
            "Fecha": period_date.dt.strftime("%Y-%m-%d"), "Tipo": "Egreso",
            ...
            "utilidad_antes_isr": 0.0, "isr": 0.0, "utilidad_neta": 0.0,
        })
    ...
    if not frames:
        return pd.DataFrame(columns=["Fecha", "Tipo", "Categoría", "Descripción", "Cantidad", "Precio_unitario", "Total", "Mes", "Año", "paga_impuestos", "iva", "subtotal", "total_con_iva", "utilidad_antes_isr", "isr", "utilidad_neta"])
...
def dashboard_tab() -> None:
    ...
    if not monthly_summary.empty:
        ...
        fig = px.bar(
            chart_df, x="year_month", y="monto", color="concepto", barmode="group",
